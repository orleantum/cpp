...
void multi(const string& inputFile, int k = 3)
{
    ...
    for (int i = 0; i < k; ++i)
        tapes[i] = "int" + to_string(i) + ".txt";

    distrub(inputFile, tapes, k);
    string tempFile = "inp.txt";
    int activTape = k;
    ...
    for (int i = 0; i < k; ++i) {
        ...
        if (f.peek() != ifstream::traits_type::eof()) {
            f.close();
            remove("in_sorted.txt");
            if (rename(tapes[i].c_str(), "in_sorted.txt") != 0)
